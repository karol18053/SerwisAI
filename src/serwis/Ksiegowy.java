...
public class Ksiegowy extends JFrame {
	...
	public Ksiegowy() {
		...
		JButton btnRozlicz = new JButton("Rozlicz");// przycisk zaloguj otwierający okno danego uzytkownika jezeli dane
													// są poprawne
		...
		JButton btnWyloguj = new JButton("Wyloguj");// przycisk zaloguj otwierajacy okno danego uzytkownika jezeli dane
													// są poprawne
...
//	}
	...
	public JButton Aktualizuj_Zakonczone_btn() {
		...
		JButton btnAktualizuj = new JButton("Aktualizuj");// przycisk zaloguj otwierajacy okno danego uzytkownika jezeli
															// dane sa poprawne
	...
	public void Aktualizuj_zakonczone() {
		...
		baza.dbConnect("jdbc:mysql://127.0.0.1:3306/serwis_baza?user=serwis&password=tornado4190");// łączenie z bazą
			...
			try {
				...
				while (rs.next()) {
					if (rs.getInt("Czy_Zaplacone") == 0) {
						wiersze[i++] = new Object[] { "FS " + rs.getString("ID_Faktura") + "/2018",
								rs.getString("Data_Wystawienia"), rs.getString("Termin_Platnosci"),
								rs.getString("Nazwa"), rs.getString("Telefon"), rs.getString("Kwota_Netto") };
					}
				}
				faktury.setModel(new DefaultTableModel(wiersze, new String[] { "Dokument", "Data powstania",
						"Termin płatności", "Nazwa", "Telefon", "Należność" }));
...
//					new Info();
//					System.out.println("Nr faktury: " + NrFaktury);
//					System.out.println("Status po: " + rs2.getInt("Czy_Zaplacone"));
				}
			}
			rs2.close();
	...
	private class PrzyciskListenerAktualizujZakonczone implements ActionListener {// klasa realizująca funkcjonalność
																					// przycisku zaloguj
	...
	private class PrzyciskListenerRozlicz implements ActionListener {// klasa realizująca funkcjonalność przycisku
																		// zaloguj
	...
	private class PrzyciskListenerWyloguj implements ActionListener {// klasa realizująca funkcjonalność przycisku
																		// zaloguj
